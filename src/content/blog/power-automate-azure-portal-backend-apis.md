...
pubDate: 2024-02-29
heroImage: "/heroes/power-automate-azure-portal-backend-apis.png"
...
My exploration revealed that during a manual CSV file upload, an API located at [main.iam.ad.ext.azure.com](http://main.iam.ad.ext.azure.com) is triggered in the background. However, this API is a closed door, accessible exclusively via the Azure portal and not through an app registration. This discovery beckoned a deeper dive to harness this API for automating the OAuth token management chore.
...
![image](/images/power-automate-azure-portal-backend-apis/01-3a9826cb-ecd9-42f8-91d2-df47a48ba54f.png)
...
A script named [01-create-resources-powershell.ps1](https://github.com/rwilson504/power-automate-azure-portal-hidden-api/blob/main/01-create-resources-powershell.ps1) is provided to facilitate the creation of a resource group and a Key Vault. It’s an executable script where you just need to replace the parameters with your environment details.
...
![image](/images/power-automate-azure-portal-backend-apis/02-dbbbe922-8346-4756-94ab-49d5be154186.png)
...
![image](/images/power-automate-azure-portal-backend-apis/03-be99a9fc-c4c7-4fc9-91fc-c78827e521ff.png)
...
For the automation to function seamlessly, acquiring a refresh token initially is crucial. This token will serve as the bridge for obtaining new access tokens, allowing the automation to interact with the [main.iam.ad.ext.azure.com](http://main.iam.ad.ext.azure.com) API securely.

A PowerShell script named [02-set-initial-refresh-token.ps1](https://github.com/rwilson504/power-automate-azure-portal-hidden-api/blob/main/02-set-initial-refresh-token.ps1) is provided to facilitate this process. When executed, this script will prompt you to sign in twice. The first login should be performed with an account that possesses the requisite permissions to update the Key Vault. Following this, a second login prompt will appear. This is where you would log in using the service account created earlier, which has been configured with just the necessary rights to interact with the Azure API. This dual-login mechanism ensures that the process is securely handled, aligning with the principle of least privilege while setting the stage for automated interactions with the [main.iam.ad.ext.azure.com](http://main.iam.ad.ext.azure.com) API. The example below shows you how to run the script, replace the parameters with your environment details.
...
![image](/images/power-automate-azure-portal-backend-apis/06-3a05e59e-d23e-40f5-8cad-bfbae5a11f60.png)

- Copy the Redirect URL from the security section, to be added within the app registration later.

![image](/images/power-automate-azure-portal-backend-apis/07-0dd6016d-b327-408e-8133-f67648bc4ce4.png)

### Update App Registration

Incorporate the Power Automate redirect URL into the app registration.

![image](/images/power-automate-azure-portal-backend-apis/08-d060a313-94b5-4cc1-994f-dce807295d1b.png)
...
![image](/images/power-automate-azure-portal-backend-apis/09-537e6ab1-3a38-4642-a883-f0868d35c17d.png)
...
For a hands-on experience, you can download a solution containing the sample Flow from here: [AzurePortalAPIFlowExample\_1\_0\_0\_4.zip](https://github.com/rwilson504/power-automate-azure-portal-hidden-api/raw/main/power-platform-solutions/AzurePortalAPIFlowExample_1_0_0_4.zip).
...
![image](/images/power-automate-azure-portal-backend-apis/10-d35adbdc-446b-416a-a609-2d5472efef6a.png)

### Get Refresh Token

The journey begins with the ‘Get secret’ action, courtesy of our custom Key Vault connector, fetching the refresh token from Key Vault to set the stage for the login process.

![image](/images/power-automate-azure-portal-backend-apis/11-7994e384-b765-434c-9358-9e10810219c7.png)
...
![image](/images/power-automate-azure-portal-backend-apis/12-6c09cffc-f8f3-467c-a4e7-2434c7a8c232.png)

### Parse JSON from Refresh Token

Simplify the subsequent steps by parsing the Key Vault response using the ‘Parse JSON’ action.

![image](/images/power-automate-azure-portal-backend-apis/13-37600217-adf0-401f-b272-63eb9ebdabc6.png)
...
![image](/images/power-automate-azure-portal-backend-apis/14-fa7a43fe-4ea2-48b3-87ee-f900e0830ea1.png)
...
![image](/images/power-automate-azure-portal-backend-apis/15-1cf15732-a132-4b80-8980-f1737ee4719d.png)
...
![image](/images/power-automate-azure-portal-backend-apis/16-aad5bb7d-c6a2-4ffa-8d5b-b8b6d743c4a3.png)
...
![image](/images/power-automate-azure-portal-backend-apis/17-ec34d95a-8a36-4555-aab7-5ecb8c0dd1ab.png)
...
In the final HTTP action of the Power Automate Flow, the above URLs and body can be employed to automate the hardware token upload process. You would specify the corresponding URL (based on your Azure environment) in the HTTP action’s URI field, and the provided JSON payload in the Body field. This automation step is crucial as it streamlines the hardware token management process by allowing a seamless upload of token data through the [main.iam.ad.ext.azure.com](http://main.iam.ad.ext.azure.com) API.
...
In conclusion, leveraging the [main.iam.ad.ext.azure.com](http://main.iam.ad.ext.azure.com) API in conjunction with Power Automate facilitates a seamless automation of hardware token management, thereby significantly reducing manual labor and enhancing security by minimizing human error.
