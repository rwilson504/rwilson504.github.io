---
title: "Copy Hidden Power Automate Expressions"
...
- This tool only works when editing flows directly at <https://make.powerautomate.com>
- It will **not** work if you’re in <https://make.powerapps.com>, because flows open in an iframe hosted on a different domain—which introduces cross-origin browser restrictions
...
(function() {
  ...
  function showDialog(expression) {
    ...
    dialog.innerHTML = `
      <div>Power Automate Expression</div>
      <input type="text" id="expression-input" value="${expression.replace(/"/g, '&quot;')}" readonly />
      <div>
        <button id="turnoff-btn">Turn Off</button>
