...
rick   -> nas.chat: "My Space (DM)"
spouse -> nas.chat: "My Space (DM)"
family -> nas.chat: "Family channel"

# ---------- Animation: trace one message round-trip ----------
scenarios: {
  "1. user types": {
    rick -> nas.chat: "✨ 'what's on my calendar?'" {
      style: { stroke: "#0969da"; stroke-width: 4; bold: true }
    }
  }
  "2. outgoing webhook fires": {
    nas.chat -> nas.oc: "✨ POST /webhook/synology" {
      style: { stroke: "#bf3989"; stroke-width: 4; bold: true }
    }
  }
  "3. openclaw replies": {
    nas.oc -> nas.chat: "✨ incoming webhook POST" {
      style: { stroke: "#1a7f37"; stroke-width: 4; bold: true }
    }
  }
  "4. user sees reply": {
    nas.chat -> rick: "✨ reply in My Space" {
      style: { stroke: "#0969da"; stroke-width: 4; bold: true }
    }
  }
}
...
The animation traces a single round-trip: you type something in your My Space → Synology Chat fires the outgoing webhook → OpenClaw processes it and replies via the incoming webhook → the message appears in your My Space. Same loop happens for the family channel, just with a different `Customize Name`.
...
Good instinct, and worth addressing because it's the first thing I tried.

Synology Chat does have a [Slash Commands](https://www.synology.com/en-us/dsm/feature/chat) feature where you register something like `/Ricky` once at the server level. It works — the command is available everywhere, looks nice in the autocomplete, and feels more "native" than trigger words on outgoing webhooks.

The problem: **slash commands don't tell the receiving webhook which channel they were invoked from.** I confirmed this experimentally. When `/Ricky hello` is sent from my My Space, the payload that arrives at OpenClaw is identical to the one that arrives when it's sent from the family channel. There's no `channel_id`, no `channel_name`, no breadcrumb.

Which means OpenClaw can't route the reply back to the channel the command came from. It defaults to the channel the slash command was originally bound to (in my case the `Family` channel, because that's where I happened to set it up first). So a `/Ricky` from my private My Space would mysteriously surface a reply in the family channel — the exact privacy violation this whole post is about avoiding.

**The takeaway:**

- If you have a **single-channel** setup (just one DM, or just one team channel), slash commands are great. Cleaner UI, easier discovery.
- If you have a **multi-channel** setup like this one, slash commands silently merge all your channels into one routing destination. Use **outgoing webhooks per channel** instead — the payload includes `channel_id` and `channel_type`, so OpenClaw can reply where the message actually came from.

That's why every step below uses outgoing webhooks, not slash commands.
